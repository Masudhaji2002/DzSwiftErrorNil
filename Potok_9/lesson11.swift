...
func buildIndex(words: [String]) -> [Entry] {
    let filteredWord = words.filter { word in
    ...
    let firstLetter = filteredWord.map { word in
        return word.prefix(1).localizedUppercase
    } // создаем массив из заглавных букв
    ...
    let entries = sortedUniquelFirstLetter.map { letter in
        let matchingWords = filteredWord.filter { word in
            return word.prefix(1).localizedUppercase == letter
        }
...
index.forEach { print("Letter: \($0.letter), Words: \($0.words)")}
